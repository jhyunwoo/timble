...
export default function Year3() {
  ...
  function get31Essential() {
    let essential
    if (group) {
      group.essentials.map((data) => {
        if (data.semester === "3-1") {
          essential = data.id
        }
      })
    }
  ...
  function get32Essential() {
    let essential
    if (group) {
      group.essentials.map((data) => {
        if (data.semester === "3-2") {
          essential = data.id
        }
      })
    }
  ...
  async function postEssential1() {
    if (group) {
      if (!get31Essential()) {
        ...
        if (selected1.length > 0) {
          setLoading(true)
          await axios.put(`/api/schools/essential?id=${get31Essential()}`, {
            data: {
              groupId: groupId(),
              subjects: makeListSubjectId1(),
              semester: "3-1",
  ...
  async function postEssential2() {
    if (group) {
      if (!get32Essential()) {
        ...
        if (selected2.length > 0) {
          setLoading(true)
          await axios.put(`/api/schools/essential?id=${get32Essential()}`, {
            data: {
              groupId: groupId(),
              subjects: makeListSubjectId2(),
              semester: "3-2",
  ...
  return (
    <ProtectedPage>
      ...
      <div className="p-4">
        <div className="text-2xl font-bold">3학년 필수 이수 과목 설정</div>
        <div className="pt-4 text-xl font-semibold">3학년 1학기</div>
        ...
        <div className="pt-4 text-xl font-semibold">3학년 2학기</div>
        <div className="grid grid-cols-2 gap-2 my-4">
          {filteredSubject2
